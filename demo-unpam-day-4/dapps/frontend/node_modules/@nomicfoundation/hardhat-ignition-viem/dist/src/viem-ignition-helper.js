...
class ViemIgnitionHelper {
    ...
    async deploy(ignitionModule, { parameters = {}, config: perDeployConfig = {}, defaultSender = undefined, strategy, strategyConfig, deploymentId: givenDeploymentId = undefined, displayUi = false, } = {
        ...
        const result = await (0, ignition_core_1.deploy)({
            ...
            maxRetries: resolvedConfig.maxRetries ?? this._hre.config.ignition.maxRetries,
            retryInterval: resolvedConfig.retryInterval ?? this._hre.config.ignition.retryInterval,
        });
    ...
    static async _toViemContracts(hre, ignitionModule, result) {
        return Object.fromEntries(await Promise.all(Object.entries(ignitionModule.results).map(async ([name, contractFuture]) => [
            name,
            await ViemIgnitionHelper._getContract(hre, contractFuture, result.contracts[contractFuture.id]),
        ])));
    }
